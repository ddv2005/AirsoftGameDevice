...
bool UBloxGPS::internalSetup()
{
    ...
    if(!ok) return false;
    ok = ublox.setNavigationFrequency(1, 3000); // Produce 4x/sec to keep the amount of time we stall in getPVT low
    if(!ok) return false;
...
void UBloxGPS::doTask()
{
    ...
    setPeriod(hasValidLocation && !wantNewLocation ? 1000 : 1000);
}
