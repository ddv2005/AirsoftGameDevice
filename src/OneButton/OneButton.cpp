 ...
 */
OneButton::OneButton(int pin, boolean activeLow, bool pullupActive, PCF8574 * expander)
{
  m_lastState = false;
  m_expander = expander;
  _pin = pin;
  ...
  if (pullupActive) {
    ...
    if(m_expander)
      m_expander->pinMode(pin,INPUT_PULLUP);
    else
      pinMode(pin, INPUT_PULLUP);
  } else {
    // use the given pin as input
    if(m_expander)
      m_expander->pinMode(pin,INPUT);
    else
      pinMode(pin, INPUT);
  } // if
} // OneButton
...
void OneButton::attachStartPress(parameterizedCallbackFunction newFunction, void* parameter)
{
  _paramStartPressFunc = newFunction;
  _startPressFuncParam = parameter;
}
 
void OneButton::attachStopPress(parameterizedCallbackFunction newFunction, void* parameter)
{
  _paramStopPressFunc = newFunction;
  _stopPressFuncParam = parameter;
}
...
{
  if (_pin >= 0) {
    bool state = (m_expander?(m_expander->digitalRead(_pin,true)?HIGH:LOW):digitalRead(_pin))  == _buttonPressed;
    if(state!=m_lastState)
    {
      m_lastState = state;
      if(m_lastState)
      {
        if (_paramStartPressFunc)
          _paramStartPressFunc(_startPressFuncParam);
      }
      else
      {
        if (_paramStopPressFunc)
          _paramStopPressFunc(_stopPressFuncParam);
      }
    }
    tick(m_lastState);
  }
